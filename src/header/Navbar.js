...
function Navbar() {
  ...
  const openProfileInNewTab = () => {
    window.open('/profile?external=true', '_blank', 'noopener,noreferrer');
  };
    ...
    <>
      <nav className='flex justify-between p-5 items-center border-b bg-[#e7e7e7d1] nav'>
        ...
        <ul className='hidden md:flex gap-6'>
          ...
          <li onClick={openProfileInNewTab} className="cursor-pointer">Profile</li>
        ...
        >
          ...
          <li onClick={openProfileInNewTab} className="hover:text-teal-700 cursor-pointer">Profile</li>
      ...
      {location.pathname !== '/profile' && token == null ? <Login setToken={setToken} /> : null}
    </>
  );
}
